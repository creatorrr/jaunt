...
Specs must be **top-level** functions or classes. The body is not used at runtime (it will be replaced/forwarded to generated code), but the *source segment + full cleaned docstring + type hints* are used as the generation contract and are hashed for incremental rebuilds. For whole-class specs, Jaunt also treats declared members and method signatures as part of the exported API seen by dependents.
...
Jaunt uses these header digests to decide whether a module is stale and needs regeneration. The exported module API digest is also used to invalidate downstream dependents when a dependency's public contract changes.
