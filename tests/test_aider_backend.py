...
def test_aider_backend_test_task_includes_bounded_coverage_guidance(monkeypatch) -> None:
    ...
    assert "invalid-input symmetry check" in contract
    assert "cache/missing-key scenario" in contract
...
def test_aider_backend_build_generation_fingerprint_excludes_test_runtime_guidance(
    monkeypatch,
) -> None:
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    monkeypatch.setattr(
...
def test_aider_test_guidance_mentions_comparison_regression_shapes() -> None:
    guidance = aider_contract_addendum("test")
    assert "invalid-input symmetry check" in guidance
    assert "cache/missing-key scenario" in guidance
