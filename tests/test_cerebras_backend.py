...
cerebras_sdk = pytest.importorskip("cerebras.cloud.sdk", reason="cerebras SDK not installed")


def test_cerebras_backend_strips_fences(monkeypatch) -> None:
    ...
    monkeypatch.setattr(backend, "_call_cerebras", fake_call)
    source, usage = asyncio.run(backend.generate_module(_ctx("build")))
    assert source == "print('hi')"


def test_cerebras_backend_renders_expected_names_and_kind_specific_rules(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
...
def test_cerebras_backend_errors_when_package_missing(monkeypatch) -> None:
    """If cerebras SDK is not installed, a clear JauntConfigError is raised."""
    monkeypatch.setenv("CEREBRAS_API_KEY", "test-key")

    original = sys.modules.get("cerebras.cloud.sdk")
    sys.modules["cerebras.cloud.sdk"] = None  # type: ignore[assignment]
...
def test_cerebras_backend_errors_when_api_key_missing(monkeypatch) -> None:
...
def test_cerebras_backend_injects_skills_block_as_extra_user_message(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
...
def test_cerebras_backend_supports_structured_output(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
    assert backend.supports_structured_output is True


def test_cerebras_generate_module_uses_structured_output(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
    ...
    monkeypatch.setattr(backend, "_call_cerebras_structured", fake_structured_call)
...
def test_cerebras_generate_module_fallback_when_structured_disabled(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
...
def test_cerebras_call_structured_sends_response_format(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
    ...
    source, usage = asyncio.run(backend._call_cerebras_structured(messages))
...
def test_cerebras_structured_json_parse_error_raises(monkeypatch) -> None:
    ...
    import jaunt.generate.cerebras_backend as mod

    monkeypatch.setattr(mod, "_BASE_BACKOFF_S", 0.001)

    with pytest.raises(json.JSONDecodeError):
        asyncio.run(backend._call_cerebras_structured([{"role": "user", "content": "hi"}]))


def test_cerebras_structured_missing_python_source_raises(monkeypatch) -> None:
    backend = _make_backend(monkeypatch)
...
def test_cerebras_structured_logs_imports_used(monkeypatch, caplog) -> None:
    backend = _make_backend(monkeypatch)
    ...
    with caplog.at_level(logging.DEBUG, logger="jaunt.generate.cerebras"):
