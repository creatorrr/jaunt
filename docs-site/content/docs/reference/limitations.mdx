...
This means build output can vary based on your environment, network connectivity, and what's installed in the active venv. Skill content does not participate in module or committed battery freshness. The selected skill set partitions response-cache entries instead. Jaunt-managed skill updates are tracked under `.jaunt/skills`; user-authored skills remain under `.agents/skills`.
