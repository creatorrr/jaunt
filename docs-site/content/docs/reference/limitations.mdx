...
This means build output can vary based on your environment, network connectivity, and what's installed in the active venv. Skill content does not participate in Python module freshness or committed TypeScript test-battery freshness — the one exception is TypeScript module sidecars, which still hash skill content into their generation fingerprint. Committing `.agents/skills/` is optional and, outside that exception, affects only prompt determinism, not staleness; it also does not stabilize content, because an installed distribution version bump rewrites the committed `SKILL.md`.
