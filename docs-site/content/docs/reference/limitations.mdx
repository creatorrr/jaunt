...
This means build output can vary based on your environment, network connectivity, and what's installed in the active venv. Skill content no longer participates in Python module freshness or committed TypeScript test-battery freshness. Committing `.agents/skills/` is optional and affects only prompt determinism, not staleness; it also does not stabilize content, because an installed distribution version bump rewrites the committed `SKILL.md`.
