...
worker_heap_mb = 6144 # optional; omit to use Node's default
...
and initialization. Both values must be positive. `worker_heap_mb` is an optional
integer of at least 256 MiB; Jaunt passes it directly to Node as
`--max-old-space-size` without accepting arbitrary `NODE_OPTIONS`. When a request times
out, Jaunt includes the worker's latest phase timing lines and names the setting to
adjust. Status and sync validate scoped dependency batches and release compiler state
between them. If one closure still exhausts the configured heap, Jaunt reports the
request once and does not replay the deterministic OOM.
