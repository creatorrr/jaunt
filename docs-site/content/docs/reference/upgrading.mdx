...
6.x. An unchanged alpha.7 workspace can reuse its saved semantic-environment
proof: preview with `jaunt migrate --language ts`, apply deterministic repairs,
then run `jaunt check --language ts`. No model call is needed when the contract,
dependencies, and compiler inputs are unchanged.
...
`jaunt-ts/1-draft.2`. That protocol is versioned for matching Jaunt Python and npm
