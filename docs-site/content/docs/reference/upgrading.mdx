...
responses after a combined compiler preflight. A later generation failure can resume
from the valid partial set, while a conflicting set never enters the cache and
committed battery files remain an all-green transaction.
