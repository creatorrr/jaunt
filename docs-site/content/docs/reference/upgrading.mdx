...
contract inputs do. A battery stamped by an older Jaunt whose contract fields
still match is re-stamped for free by the next `jaunt test --language ts`, with
no model call and no API key. `jaunt check` now agrees with `jaunt test`: it
reports non-gating divergence as an advisory on the diagnostic instead of
failing the gate. This covers TypeScript *test batteries*; module sidecars still
hash skill content into their generation fingerprint.
...
`check` ignore it. A separate battery advisory channel is future work.
