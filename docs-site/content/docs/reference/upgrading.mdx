...
provenance still fails `check`. A battery stamped by an older Jaunt whose contract
fields still match is re-stamped for free by the next `jaunt test --language ts`, with
no model call and no API key. `jaunt check` now agrees with `jaunt test`: it reports
non-gating divergence as an advisory on the diagnostic instead of failing the gate.
...
`imported_type_context_fingerprint` for reasons unrelated to this fix. That
