...
**Compatibility promise:** patch releases (1.3.0 → 1.3.1) never invalidate
built modules — your generated code stays fresh across the upgrade. Anything
that would restale built modules lands in a minor version and is called out on
this page.
...
- **Migrating inner `@jaunt.magic` to `@jaunt.sig` rebuilds that module once.**
  The alias keeps working, but the decorator's identity is part of the class's
  structural contract, so the rename restales the spec. Budget one rebuild per
  whole-class module when you adopt the new vocabulary.
