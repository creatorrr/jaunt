...
A stale or exhausted TypeScript battery carries three views of the same divergence in
the `data` mapping of its structured diagnostic: `mismatches`, `gating`, and
`advisories`. These keys appear only on `JAUNT_TS_TEST_BATTERY_STALE` and
`JAUNT_TS_TEST_GENERATION_EXHAUSTED`; `JAUNT_TS_TEST_BATTERY_MISSING` carries none of
them. `mismatches` is every observed provenance divergence and is always present.
`gating` is the subset that failed the check; it is always present and never empty,
