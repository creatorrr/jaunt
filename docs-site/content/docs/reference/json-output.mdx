...
environment identities differ. Mixed workspaces place it under `targets.ts`.
Older sidecars may have only the aggregate digest; in that case the availability
flags and record counts explain why no exact path diff is possible.
