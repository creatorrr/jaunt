...
belong to `build`; compatible artifact repairs belong to `migrate --language
ts`. A synchronized placeholder keeps the editor green but remains `unbuilt` in
`status` and `check`. Full-workspace sync validates dependency-ordered bounded batches
and commits them together. With `--target`, contract analysis and overlay validation
include only the selected module, its explicit and public-import closure through
ordinary barrels/context files, and configured global-declaration roots.
...
| `--instruction "..."` | Extra generation instruction (repeatable). On `jaunt test --language ts`, it reaches both example and derived battery prompts. |
...
When TypeScript status reports structural drift together with
`semantic_environment_changes`, preview `jaunt migrate --language ts --json`
before approving a build. The explicit migration can validate and recompose an
unchanged implementation for free; normal `build` stays conservative.

### `jaunt doctor`

Run read-only environment and workspace checks. Doctor does not build, mutate
artifacts, or call a model.

```bash
jaunt doctor --json
```

Alongside status and toolchain results, `environment.jaunt` reports the running
version, resolved entrypoint, loaded CLI module, Python executable,
distribution root or editable `direct_url`, nearest `uv.lock`, and its locked
Jaunt requirement. A running/locked mismatch is a finding because the next
`uv sync` may replace the active install. `distribution_matches_module` is true
only when that metadata resolves to the loaded CLI module; otherwise doctor
reports every candidate and tells you to trust the explicit `module` path.
...
jaunt test --language ts --no-build --instruction "Use the canonical fixture factory"
```
...
| `--instruction "..."` | Extra generation guidance (repeatable). TypeScript passes it to example and derived battery prompts; the one-off value is not committed provenance. |
...
battery and appear under `refrozen`. When `target_api_digest` and the aggregate battery
fingerprint changed, Jaunt first applies the current safety scan, then typechecks and
runs the committed body against the current target. This free route also accepts
co-drift in the prompt, runner, or Vitest fingerprint. A green body is reheadered
without a model call, while a behavioral or compiler failure falls back to generation.
`--no-run` disables this verification.
Other content-affecting drift and `--force` regenerate the battery.
When a separate `jaunt build` has already validated a compatible API recomposition,
the following test command reheaders matching batteries from the persisted local proof.
Build JSON uses `refrozen` for the full model-free set and `recomposed` for the
compatible-toolchain subset.
...
Manage user skills under `.agents/skills/` and Jaunt-managed skills under
`.jaunt/skills/`.
...
jaunt skill refresh
jaunt skill migrate       # preview classic managed-skill migration
...
regenerates Jaunt-managed auto skills. `skill migrate` is deterministic and moves
only provenance-marked managed skills; it never calls a model or touches manual
skills.
...
- recompose an environment-only structural change after the saved candidate
  passes current compiler, policy, API, and consumer validation;
- report `model-rebuild` when the contract changed or a protocol, IR, or
...
For provenance recovery, run the JSON dry plan first and verify that each
affected action is `free-recompose` and `requires_rebuild` is empty. After the
apply, run `jaunt test --language ts --no-build` and `jaunt check --language ts`.
Migration records the validated API transition so compatible batteries can be
reheadered without a model call. A failed overlay writes nothing.

Use repeatable `--target ts:<module-id>` flags to preview or apply only selected
modules. Targeted plans may analyze dependency context, but their action and
write sets contain no unselected module artifacts.
...
| `--target ts:<module-id>` | Restrict a TypeScript plan/apply to selected modules. Repeatable. |
