...
`magic` block (`fresh` / `stale` / `unbuilt`, plus `orphans`); see
...
Apply pending mechanical migrations to your spec source. This is the one jaunt
command that edits files you wrote, so it plans by default: `jaunt migrate`
prints what it would change and exits `0`, and `jaunt migrate --apply` writes the
changes. Neither mode calls the model.
...
| `--json` | Emit `{applied, actions: [...]}`; each action carries `migration`, `path`, `module`, `symbol`, `kind`, `classification`, and `description`. |
