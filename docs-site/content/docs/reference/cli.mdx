...
facades are never overwritten, and sync never changes a built implementation's body
or public boundary. A semantic refreeze or fingerprint re-stamp may update only its
Jaunt provenance header so the implementation and sidecar remain consistent. A
synchronized placeholder keeps the editor green but remains `unbuilt` in `status` and
`check`.
