...
| **Sealed** | inner `@jaunt.magic` | stub | implement the body only; the signature (params, defaults, annotations, return type) is enforced **exactly** |
...
`@jaunt.magic` on a method *inside* a `@jaunt.magic()` class means exactly what it means everywhere else — "Jaunt implements this, as declared" — so the declared signature becomes a lock:
...
Sealed-signature drift is a hard build error; guidepost drift is only a warning. Inner `@jaunt.magic` inside a whole-class spec takes no kwargs and cannot sit under `@property` (v1) — use a guidepost stub or `@jaunt.preserve` instead.
