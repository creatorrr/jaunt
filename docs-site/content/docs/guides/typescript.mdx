...
`[test].jobs` or `--jobs`. Completed responses pass a combined compiler preflight
before entering the cache; a late request failure also preflights and stages its valid
partial set. A late generation or runner failure commits no battery files, but a rerun
can reuse staged paid responses after validating them again. Plain progress reports
each path and tier as it is validated and staged, and JSON includes the effective
`jobs` value plus a `batteries` outcome for every path.
