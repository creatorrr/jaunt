...
full-project overlay and static-loader policy; its exact diagnostics enter the
remaining retry budget. Typecheck-only messages are bounded to 2,000 characters;
executed derived failures still expose only an opaque case ID and category. A late
request failure also preflights and stages its valid partial set. If separately valid
batteries conflict when combined, Jaunt finds a deterministic compatible subset, runs
it, and commits it only when that subset is green. Conflicting paths remain
uncommitted and the command exits `3`. A rejected cached candidate is removed before
the next run. Plain progress reports each path and tier, while JSON includes
`attempts`, `retry_count`, `retry_reasons`, any terminal `rejection_reasons`, and
`cache_evicted` when a cached response was rejected.
...
environment identity. On a later compatible Jaunt or worker upgrade, Jaunt compares
that identity and the saved contract through its dependency closure. If only Jaunt
tool metadata or composition details changed, it recovers the existing candidate,
runs the current compiler and validation pipeline, recomposes the deterministic public
boundary, and reheaders affected test batteries without a model call. Older sidecars
without this proof rebuild once. A changed external declaration, non-tooling lockfile
entry, or dependency contract remains a normal structural rebuild.
