...
and overlay compiler validation include the selected module and its dependency
closure, not unrelated configured sources. A broad stable root or glob can therefore
remain in `jaunt.toml`; you should not need to rotate roots or `tsconfig` includes as
...
reference are separate write transactions. If one candidate fails, validated
...
not trigger an implementation repair. A failed run commits neither batteries nor
generation-cache entries; rerunning regenerates the uncommitted candidates.
...
Jaunt keeps that accepted candidate in the response cache. A retry with the same
project identity revalidates the cached source before use and does not call the model
again.
...
current alpha. Keep them in the paired context module, or keep the public module as a
...
inspect the planned file count and bytes. The current alpha plans all installed direct
