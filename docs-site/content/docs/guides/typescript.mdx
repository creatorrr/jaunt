...
remaining retry budget. A late request failure also preflights and stages its valid
partial set. If separately valid batteries conflict when combined, Jaunt finds a
deterministic compatible subset, runs it, and commits it only when that subset is
green. Conflicting paths remain uncommitted and the command exits `3`. Plain progress
reports each path and tier, while JSON includes `attempts`, `retry_count`,
`retry_reasons`, and any terminal `rejection_reasons` in each `batteries` outcome.
