...
without a plugin or a server to install. Everything the agent needs is on the
CLI: a primer to load into context, structured output on every command it
drives, and exit codes it can branch on.
...
points the agent back at the owning spec. Wire it into Claude Code via
