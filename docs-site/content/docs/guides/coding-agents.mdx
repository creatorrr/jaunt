...
without a plugin or a server to install. If you are on Claude Code, there is
also a first-party plugin that packages all of this — the guard hook, the
session freshness map, and the build/convert/doctor skills: see
[the Claude Code plugin](/docs/guides/claude-code-plugin). Everything the agent
...
[`jaunt` Claude Code plugin](/docs/guides/claude-code-plugin) ships this hook
prewired, with owning-project resolution so `generated_dir` comes from the right
config in a multi-project repo. Or wire it in yourself via `.claude/settings.json`:
