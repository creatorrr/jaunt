...
A stub body is a placeholder Jaunt never runs. Use `...`:
...
`pass`, a bare docstring, and `raise NotImplementedError` all read as stubs too. The older `raise RuntimeError("spec stub")` convention does **not** — in a governed module that body looks like real code, so the function classifies as handwritten. Standardize on `...`.
