...
## Workspaces: one jaunt project per package

In a uv workspace, give each adopted package its own `jaunt.toml` with
`source_roots = ["."]` at that package's import root, and run jaunt from there.
Everything that resolves against the first source root then lands correctly:
where generated files are written, what `jaunt check` reads, and which pyproject
the undeclared-import validator consults. A single root config that lists more
than one source root is a hard error as of 1.5.1 — jaunt used to route all
output to the first existing root and silently break the others.

One thing to watch across split configs: keep the `[codex]` block and
`[build] instructions` byte-identical in every package's `jaunt.toml`. Both feed
the generation fingerprint, so a stray difference restales — and re-bills —
every module in that project. Shared guidance now lives once per project rather
than once per repo; a config include would remove the footgun, but it does not
exist yet.
