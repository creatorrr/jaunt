...
A spec is a normal function with a real signature and a docstring that states the contract. The body stays empty. Because `magic_module` governs the file, `create_token` is a spec on its own. `verify_token` carries a `@jaunt.magic` decorator only to declare a dependency; the decorator is the precision layer for a single symbol, and it still inherits whatever the module call set. Put this in `src/specs.py`:
