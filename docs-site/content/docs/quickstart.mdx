...
If you’re adding Jaunt to your own project, install it however you vend it
(local path, internal index, or git). The base install is batteries-included:

```bash
# Example (from package index):
uv add jaunt   # includes pytest, pytest-asyncio, anyio, rich, and watchfiles
```
