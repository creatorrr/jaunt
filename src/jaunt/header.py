...
STUB_HEADER_MARKER = "# This .pyi stub was generated by jaunt. DO NOT EDIT."
...
def parse_header(source: str) -> dict[str, str] | None:
    ...
    Returns ``None`` when *source* does not begin with :data:`HEADER_MARKER`
    (including empty input). Otherwise scans the immediately following
    ``# jaunt:key=value`` comment lines, stopping at the first line that does
    not start with ``# jaunt:``. Each parsed line is split on the first ``"="``;
    the key and value are stripped. A file whose first line is the marker but
    which has no following ``# jaunt:`` lines yields an empty dict.

    Examples:
        - parse_header('') == None
        - parse_header(HEADER_MARKER) == {}
...
@jaunt.contract
def parse_stub_header(source: str) -> dict[str, str] | None:
    """Parse the Jaunt stub header from the start of a file.

    Behaves exactly like :func:`parse_header` but keyed off
    :data:`STUB_HEADER_MARKER`: returns ``None`` when *source* does not begin
    with the stub marker (including empty input), otherwise returns the
    ``# jaunt:key=value`` mapping that follows it (empty dict when the marker
    stands alone).

    Examples:
        - parse_stub_header('') == None
        - parse_stub_header(STUB_HEADER_MARKER + '\\n# jaunt:kind=stub') == {'kind': 'stub'}
    """

    lines = source.splitlines()
    if not lines:
        return None
    if lines[0] != STUB_HEADER_MARKER:
        return None
...
def extract_stub_generated_digest(source: str) -> str | None:
    parsed = parse_stub_header(source)
    if parsed is None:
        return None
    return parsed.get("generated_digest")
...
def extract_digest_scheme(source: str) -> int | None:
    """Return the ``digest_scheme`` header value as an int, or ``None``.

    Parses *source* with :func:`parse_header`. Returns ``None`` when there is no
    Jaunt header, when the header carries no ``digest_scheme`` key, or when the
    value is not a base-10 integer. A well-formed integer value is returned as
    an ``int``.

    Examples:
        - extract_digest_scheme('') == None
        - extract_digest_scheme(HEADER_MARKER + '\\n# jaunt:digest_scheme=2') == 2
