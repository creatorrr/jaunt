...
QuotaResultT = TypeVar("QuotaResultT")
...
class _QuotaRetryState:
    ...
    on_exhausted: Callable[[], None] | None = field(default=None, repr=False)
    _lock: threading.Lock = field(default_factory=threading.Lock, init=False, repr=False)

    def reserve_wait(self) -> tuple[float, float] | None:
        """Reserve one backoff delay from the command-wide wait budget.

        Reservation happens before sleeping so concurrent artifacts, including mixed
        Python/TypeScript work running on different event loops, can never overdraw the
        configured total. A cancelled sleep remains charged conservatively.
        """

        with self._lock:
            if self.remaining_seconds <= 0:
                return None
            delay = min(self.next_delay_seconds, self.remaining_seconds)
            self.remaining_seconds -= delay
            self.next_delay_seconds = min(
                self.next_delay_seconds * 2,
                _MAX_QUOTA_WAIT_SECONDS,
            )
            return delay, self.remaining_seconds

    def notify_exhausted(self) -> None:
        """Abort sibling command work when the shared wait budget is spent."""

        if self.on_exhausted is not None:
            self.on_exhausted()


_QUOTA_RETRY_STATE_INIT_LOCK = threading.Lock()


async def _call_with_quota_retry(
    call: Callable[[], Awaitable[QuotaResultT]],
    *,
    progress: Callable[[str, str], None] | None,
    quota: _QuotaRetryState,
) -> QuotaResultT:
    """Retry one model call against a shared, reserve-before-sleep quota budget."""

    while True:
        try:
            return await call()
        except JauntQuotaGenerationError as exc:
            reservation = quota.reserve_wait()
            if reservation is None:
                quota.notify_exhausted()
                raise
            delay, remaining = reservation
            if progress is not None:
                progress(
                    "quota-wait",
                    f"waiting {delay / 60:g} minute(s); {remaining / 60:g} minute(s) remain: {exc}",
                )
...
async def _call_with_infrastructure_retry(
    ...
    *,
    ...
    while True:
        try:
            return (
                await _call_with_quota_retry(call, progress=progress, quota=quota),
                tuple(errors),
            )
...
class GeneratorBackend(ABC):
    ...
    def _quota_retry_state(self) -> _QuotaRetryState:
        """Return this command-owned backend's shared quota retry state.

        Backend subclasses are not required to call ``super().__init__``, so the
        state is initialized lazily. The CLI creates a fresh backend for each command;
        every artifact generated by that backend shares this one bounded budget.
        """

        state = getattr(self, "_jaunt_quota_retry_state", None)
        if isinstance(state, _QuotaRetryState):
            return state
        with _QUOTA_RETRY_STATE_INIT_LOCK:
            state = getattr(self, "_jaunt_quota_retry_state", None)
            if not isinstance(state, _QuotaRetryState):
                state = _QuotaRetryState(max(0.0, self.quota_wait_minutes * 60.0))
                self._jaunt_quota_retry_state = state
        return state

    async def run_with_quota_retry(
        self,
        call: Callable[[], Awaitable[QuotaResultT]],
        *,
        progress: Callable[[str, str], None] | None = None,
    ) -> QuotaResultT:
        """Run an auxiliary model call against this backend's command-wide budget."""

        return await _call_with_quota_retry(
            call,
            progress=progress,
            quota=self._quota_retry_state(),
        )
    ...
    async def complete_text_with_usage_and_quota_retry(
        ...
        *,
        ...
        """Complete text while sharing the backend's command-wide quota wait budget."""

        return await self.run_with_quota_retry(
            lambda: self.complete_text_with_usage(system=system, user=user),
            progress=progress,
        )
