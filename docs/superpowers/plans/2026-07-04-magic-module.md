...
- **Landing (`page.tsx` + `index.mdx`):** hero sample becomes the magic_module email-parser example (spec §1's exact shape); decorators appear as the second beat ("need per-symbol control? decorate just that symbol").
...
- **`concepts/how-jaunt-works.mdx`:** add the two-phase registration story (AST-register at call time → post-import backfill) and Approach-A forwarding (intercept once, rebind, vanish; `__class__` swap-back).
- **`concepts/change-detection.mdx`:** digest neutrality iff body form unchanged; `raise RuntimeError` bodies restale on conversion; module `prompt=` edits restale every governed spec (correct, it's their contract).
